...
impl AsInputReport for CodexReport {
    fn serialize(&self, buffer: &mut [u8]) -> Result<usize, usbd_hid::descriptor::BufferOverflow> {
...
mod ble_report_map_tests {
    ...
    fn ble_report_map_matches_service_definition() {
        ...
        assert_eq!(desc.len(), 227, "update HidService's report_map size on change");
...
pub enum Report {
    ...
    /// Codex Micro-compatible vendor notification (BLE Report ID 6).
...
impl AsInputReport for Report {
    fn serialize(&self, buffer: &mut [u8]) -> Result<usize, usbd_hid::descriptor::BufferOverflow> {
        match self {
            ...
            Report::CodexReport(r) => r.serialize(buffer),
            #[cfg(feature = "steno")]
