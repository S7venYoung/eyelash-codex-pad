...
async fn gatt_events_task(server: &Server<'_>, conn: &GattConnection<'_, '_, DefaultPacketPool>) -> Result<(), Error> {
    ...
    let codex_input = server.hid_service.codex_input;
    let codex_output = server.hid_service.codex_output;
    let codex_feature = server.hid_service.codex_feature;
                ...
                let result = match &gatt_event {
                    ...
                    GattEvent::Write(event) => {
                        ...
                        if event.handle() == output_keyboard.handle {
                            ...
                            || event.handle() == codex_input.cccd_handle.expect("No CCCD for Codex report")
                            || event.handle() == level.cccd_handle.expect("No CCCD for battery level")
                        {
                            ...
                            if event.handle() == codex_output.handle || event.handle() == codex_feature.handle {
                                // Host-to-device Codex RPC is reserved for the
                                // next milestone. Accepting the report keeps the
                                // characteristic interoperable while input-only
                                // firmware is exercised safely.
                                debug!("Got Codex vendor packet: {:?}", data);
                            } else {
                            #[cfg(feature = "host")]
